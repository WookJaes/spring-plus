...
최적화 적용 전에는 별도의 닉네임 인덱스나 캐시를 적용하지 않고,
Spring Data JPA 메서드 쿼리로 `User` 엔티티를 조회했다.
...
- 여러 서버 인스턴스에서 캐시를 공유해야 하는 운영 환경에서는 Redis와 같은 분산 캐시 도입을 고려할 수 있다.
...
- 기본적인 검색 성능은 인덱스로 확보하고, 반복 조회가 많은 데이터에 한해 캐시를 함께 적용하는 방식이 적절하다고 판단했다.
